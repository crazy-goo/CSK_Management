...
const AuthShell = ({ eyebrow, title, description, children, footer }) => {
  return (
    <div className="min-h-screen bg-(--color-cream) px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-7rem)] max-w-6xl items-center gap-6 lg:grid-cols-[1fr_480px]">
        <div className="minimal-panel hidden min-h-130 flex-col justify-between lg:flex">
          <div className="space-y-5">
            <p className="font-script text-2xl text-(--color-accent-strong)">
              {eyebrow}
            </p>
            <h1 className="max-w-lg text-5xl font-semibold leading-tight text-(--color-ink)">
              {title}
            </h1>
        ...
        <div className="rounded-4xl border border-black/10 bg-white p-6 shadow-[0_24px_60px_rgba(32,24,12,0.08)] sm:p-8">
          <div className="mb-8 space-y-3 lg:hidden">
            <p className="font-script text-2xl text-(--color-accent-strong)">
              {eyebrow}
            </p>
            <h1 className="text-3xl font-semibold leading-tight text-(--color-ink)">
              {title}
            </h1>
